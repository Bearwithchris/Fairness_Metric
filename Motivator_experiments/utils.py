...
def populate_Lgt(Tperc,k,Nsamples=3000,skew=0):
    ...
    sample_array.append(bias_samples)
    if skew==0:
        #Uniform remaining
    ...
    for i in range (0,k-1):
        ...
        sample_array.append(__samples)
        bias_samples=np.concatenate([bias_samples,__samples])
    return (bias_samples,sample_array)


#Generate the predicted labels, simulating the classifier of a given accuracy Tpre
def gen_pred_labels(k,ibias,Lgt,Nsamples,u_acc,bias_flip=0):
    # start=time.time()
    # Lgt=populate_Lgt(ibias,k,Nsamples) #Label ground truth
    Lpred=copy.deepcopy(Lgt[1])
    # print ("Acc_avg={}".format(sum(u_acc)/len(u_acc)))
    
    #iterrating through accuracies
    for i in range(len(u_acc)):
        #Create prob distribution for TP-TN
        prob_dist=np.ones(k)*((1.0-u_acc[i])/(k-1))
        prob_dist[i]=u_acc[i]
        
        #iterrating through individual labels
        for j in range(len(Lpred[i])):
            #"classifier"
            predict=np.random.choice(np.arange(0.0,k,1),p=prob_dist)
            Lpred[i][j]=predict
